...
function Orders() {
  ...
  const getOrders = async (page = 1) => {
    try {
      const { data } = await axios.get(
        ...
        }/api/Orders?state=${filterState}&page=${page}&pageSize=${
          pagination.pageSize
        }`,
        {
          headers: {
  ...
  const handleStateChange = (orderId, value) => {
    ...
    changeOrderState(orderId, value); // Call the API to update the state
  };
